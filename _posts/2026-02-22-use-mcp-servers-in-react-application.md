...
date: 2026-02-22 14:40:00 -0400
author: "João Mendes"
categories: [react, mcp]
tags: [React, MCP, Model Context Protocol, Microsoft Graph, TypeScript]
excerpt: "Learn how to connect a React app to an MCP server that wraps Microsoft Graph, using custom hooks, MSAL authentication, infinite scroll pagination, and more."
image: /assets/images/posts/use-mcp-servers-in-react-application/thumbnail.png
---

![Using MCP Servers in React Applications — React logo connected to MCP Server connected to Microsoft 365]({{ "/assets/images/posts/use-mcp-servers-in-react-application/thumbnail.png" | relative_url }})
...
![My Team component showing João Mendes as top manger with a grid of 16 direct reports]({{ "/assets/images/posts/use-mcp-servers-in-react-application/MyTeam.png" | relative_url }})
...
![Architecture diagram showing the flow from the MyTeam React component through BrowserHTTPTransport to the MCP Server tools, then to Microsoft Graph API endpoints, and finally to Microsoft 365]({{ "/assets/images/posts/use-mcp-servers-in-react-application/architecture.png" | relative_url }})
...
- **useState** — `currentUser`, `directReports`, `skipToken`, `hasNextPage`, `isLoading`, `isLoadingMore`, `isFetching`, `containerHeight`, `error`
- **useEffect** — Mount the `ResizeObserver`; trigger initial data load once
- **useCallback** — Stable references for `parseDirectReports`, `fetchDirectReports`, `handleLoadMore`, `handleScroll`, and action handlers
- **useMemo** — Calculate `pageSize` from `containerHeight`
- **useRef** — `containerRef` (DOM node), `hasFetchedRef` (double-fetch guard)
- **React.memo** — Skip re-renders when props have not changed

### useMCPClient hook

- **useRef** — `clientRef`, `transportRef`, `pendingRequestsRef`
- **useCallback** — `connect`, `disconnect`, `listTools`, `callTool`, `selectTool`, `clearCache`
- **useEffect** — Disconnect and clean up on unmount

### useAuth hook

- **useCallback** — `acquireToken`, `login`, `logout`, `isTokenExpiringSoon`
- **useRef** — `tokenExpiryRef`, `isRefreshingRef`
- **useEffect** — Background token refresh interval

### useLogging hook

- **useCallback** — All `logX` functions
